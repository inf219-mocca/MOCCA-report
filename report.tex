...
libraries available and fairly consise. 
The choice of Django however was a bit more contentious, in the very beginning 
the battle plan was to use Flask which would've meant that we'd have to do a 
lot of the heavy lifting ourselves.
...
power it's size and affordability makes it a perfect data cruncher. We used 
Raspberry Pi's very own camera module for ease of 
use and a library is included. The module itself is plug-and-play and has more 
than high enough resolution for our use. Since we analyse the pixels in an 
